...
const InputSearch = () => {
  ...
  const handleKeyPress = (e) => {
    if (e.key === "Enter") {
      setLoading(true);
      axios
        .get(`http://www.too-sport.com/api/search/football/${submitValue.value}`)
        .then((res) => {
    ...
    <div className="newsman-block">
      ...
      <div className="newsman-block-content">
        <div className="newsman-search">
          ...
          <input
            type="text"
            placeholder="Selecciona el filtro de busqueda"
            className="input"
        ...
        <div className="tags display-flex">
            <div className="newsman-badge">
              <a href="!" className="badge bg-color-white text-color-black">Partidos de Futbol</a>
            </div>
            <div className="newsman-badge">
              <a href="!" className="badge bg-color-white text-color-black">Canales</a>
            </div>
          </div>
      </div>
      { football !== null ? <ResultSearch data={football} state={loading} /> : null}
    </div>
  );
};
